...
import { useNPCs, useCreateNPC, useUpdateNPC, useDeleteNPC } from "../hooks/useNpc";
...
export const AdminNPCsPage = () => {
  ...
  const handleEdit = (n: NPC) => {
    setEditingId(n.id);
    setName(n.name);
    setWorldId(String(n.worldId));
    setShowForm(true);
  };
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    try {
      if (editingId) {
        await updateMutation.mutateAsync({ id: editingId, data: { name, worldId: Number(worldId) } });
      } else {
        await createMutation.mutateAsync({ name, worldId: Number(worldId) });
      }
      resetForm();
    } catch {
    ...
    >
      ...
      <div style={{ flex: 1, padding: 32, overflow: "auto" }}>
        <div style={{ maxWidth: 1100, margin: "0 auto" }}>
          <h1 style={{ fontSize: 28, fontWeight: 700, marginBottom: 32 }}>Manage NPCs</h1>
          ...
          {showForm && (
            <form onSubmit={handleSubmit} style={{ padding: 24, borderRadius: 12, background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.04)", marginBottom: 24 }}>
              <h2 style={{ marginBottom: 20, fontSize: 18, fontWeight: 600 }}>{editingId ? "Edit NPC" : "New NPC"}</h2>
              <div style={{ marginBottom: 16 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#888", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>Name</label>
                <input value={name} onChange={e => setName(e.target.value)} required style={inputStyle} onFocus={(e) => { e.target.style.borderColor = "#00c853"; e.target.style.boxShadow = "0 0 0 3px rgba(0,200,83,0.12)"; }} onBlur={(e) => { e.target.style.borderColor = "rgba(255,255,255,0.06)"; e.target.style.boxShadow = "none"; }} />
              </div>
              <div style={{ marginBottom: 20 }}>
                <label style={{ display: "block", fontSize: 12, fontWeight: 500, color: "#888", marginBottom: 6, textTransform: "uppercase", letterSpacing: "0.5px" }}>World ID</label>
                <input value={worldId} onChange={e => setWorldId(e.target.value)} required style={inputStyle} onFocus={(e) => { e.target.style.borderColor = "#00c853"; e.target.style.boxShadow = "0 0 0 3px rgba(0,200,83,0.12)"; }} onBlur={(e) => { e.target.style.borderColor = "rgba(255,255,255,0.06)"; e.target.style.boxShadow = "none"; }} />
              </div>
          ...
          {isLoading && (
            <div style={{ textAlign: "center", padding: 40, color: "#888" }}>
              <div style={{ marginBottom: 12 }}>Loading NPCs...</div>
            </div>
          )}

          {npcs && npcs.length === 0 && !isLoading && (
            <div style={{ textAlign: "center", padding: 60, color: "#888" }}>
              ...
              <p style={{ margin: 0 }}>No NPCs yet.</p>
            </div>
          )}

          <div style={{ display: "grid", gap: 12 }}>
            {npcs?.map(n => (
              <div key={n.id} style={{ padding: 20, borderRadius: 12, background: "#1a1a1a", border: "1px solid rgba(255,255,255,0.04)", display: "flex", justifyContent: "space-between", alignItems: "center", transition: "border-color 0.3s", cursor: "default" }} onMouseEnter={(e) => { e.currentTarget.style.borderColor = "rgba(0,200,83,0.12)"; }} onMouseLeave={(e) => { e.currentTarget.style.borderColor = "rgba(255,255,255,0.04)"; }}>
                <div>
                  <h3 style={{ margin: 0, marginBottom: 4, fontSize: 16, fontWeight: 600 }}>{n.name}</h3>
                  <p style={{ margin: 0, color: "#888", fontSize: 14 }}>World: {n.worldId}</p>
                </div>
                <div style={{ display: "flex", gap: 8 }}>
                  <Button onClick={() => handleEdit(n)} variant="secondary" height={36} fontSize={13}>Edit</Button>
                  <Button onClick={() => handleDelete(n.id)} variant="secondary" height={36} fontSize={13} disabled={deleteMutation.isPending}>Delete</Button>
                </div>
              </div>
            ))}
          </div>
